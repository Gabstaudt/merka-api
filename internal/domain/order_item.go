...
type OrderItem struct {
	...
	TenantID      uuid.UUID
	ComandaID     uuid.UUID
	AtendimentoID *uuid.UUID // ciclo de uso da comanda em que este item foi lançado — ver domain.Atendimento
	ProductID     uuid.UUID
...
// dispositivo.
func NovoOrderItemPeso(tenantID, comandaID uuid.UUID, atendimentoID *uuid.UUID, product *Product, pesoBruto float64, lancadoPor uuid.UUID) *OrderItem {
...
func NovoOrderItemUnitario(tenantID, comandaID uuid.UUID, atendimentoID *uuid.UUID, product *Product, quantidade float64, lancadoPor uuid.UUID) *OrderItem {
